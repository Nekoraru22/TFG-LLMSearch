...
class ChromaClient:
    ...
    def add_documents(self, documents: list, embeddings=None, metadatos=None, ids=None) -> None:
        """
        ...
        Args:
            ...
            metadatos: List of metadata (dictionaries) for each document
        ...
        if metadatos is None:
            metadatos = [{"source": "example"} for _ in range(len(documents))]  # Ensure keys and values match Metadata type

        if embeddings is None:
            # Use embeddings automatically generated by Chroma
            self.collection.add(
                documents=documents,
                metadatas=[{k: v for k, v in metadata.items()} for metadata in metadatos],
                ids=ids
            )
        else:
